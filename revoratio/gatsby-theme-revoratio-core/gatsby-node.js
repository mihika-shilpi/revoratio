...
exports.createPages = (
  ...
  return graphql(
    ...
    if (!repositoryUrl && githubUrl) {
      reporter.warn(
        `gatsby-theme-revoratio: The option \`githubUrl\` was deprecated in favor of \`repositoryUrl\`. \nTo remove this warning, replace \`githubUrl\` with \`repositoryUrl\`.`,
      );
    }
...
exports.onPreBootstrap = ({ store, reporter }, themeOptions) => {
  const { configPath, docsPath, imgPath } = withDefault(themeOptions);
  const { program } = store.getState();

  const dirs = [
    path.join(program.directory, configPath),
    path.join(program.directory, docsPath),
    path.join(program.directory, imgPath),
  ];
